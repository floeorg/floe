...
class TestCompletionPipe:
    def test_after_pipe_has_items(self, lsp):
        ...
        labels = completion_labels(lsp.completion(URI, 1, len("result = nums |> ")))
        assert len(labels) > 0
    ...
    def test_string_module_methods(self, lsp):
        open_doc(lsp, URI, 's = "hello"\nconst r = s |> String.\n')
        labels = completion_labels(lsp.completion(URI, 1, len("r = s |> String.")))
...
class TestCompletionPipeFiltering:
    ...
    def test_pipe_for_block_functions(self, lsp):
        ...
        open_doc(lsp, URI, source)
        labels = completion_labels(lsp.completion(URI, 9, len("r = todos |> ")))
    ...
    def test_pipe_string_bare_names(self, lsp):
        ...
        open_doc(lsp, URI, 's = "hello"\nconst r = s |> \n')
        labels = completion_labels(lsp.completion(URI, 1, len("r = s |> ")))
