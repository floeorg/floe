...
pub fn register(fns: &mut Vec<StdlibFn>) {
    ...
    fns.extend([
        ...
        stdlib_fn!("Array", "indexOf", [array_of(t.clone()), t.clone()], option_of(Type::Number), "(() => { const _i = $0.indexOf($1); return _i === -1 ? undefined : _i; })()"),
        ...
        stdlib_fn!("Array", "uniqueBy", [array_of(t.clone()), fun(vec![t.clone()], u.clone())], array_of(t.clone()), "(() => { const _a = $0; const _f = $1; const _s = new Set(); return _a.filter(_v => { const _k = _f(_v); if (_s.has(_k)) return false; _s.add(_k); return true; }); })()"),
