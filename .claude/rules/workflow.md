...
**When implementation is done, run `/ship`.** It handles everything: quality gates, `/simplify`, `/rulify`, draft PR, CI loop (including merge conflicts), and mark ready. After the user merges, say "merged" to trigger `/land`.
